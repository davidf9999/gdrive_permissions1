...
* **שימוש:** יש להזין **כתובת אימייל חוקית אחת בדיוק בכל שורה** בעמודה A (`Email`). כותרת זו תואמת לשם העמודה שייבוא אנשי הקשר של Google מצפה לו (`Email`), כך שניתן לייצא או להעתיק גיליון משתמשים לייבוא אנשי קשר ללא שינוי שמות עמודות. אם תא מכיל מספר כתובות או כל דבר אחר שאינו כתובת אימייל בודדת וחוקית, הסקריפט יתעד שגיאה ויתעלם מהרשומה. עמודה B האופציונלית (`Disabled`) מאפשרת לך להחריג משתמש באופן זמני מקבלת הרשאות — הגדר אותה ל-`TRUE`, `YES`, או סמן את התיבה כדי להשבית את המשתמש מבלי למחוק את השורה שלו. כותרות נוספות התואמות לייבוא/ייצוא אנשי קשר זמינות לנוחות: `Name`, `Given Name`, `Family Name`, `Phone`, `Notes`, `Address`, `Organization`, `Title`, `Birthday`, ו-`Group Membership`.
    *   **הוספת כתובות או קישורי מפה:** Google Contacts מצפה שהעמודה `Address` תכיל כתובת פיזית (רחוב, עיר וכו'). בזמן הייבוא הטקסט הופך אוטומטית לקישור מפה לחיץ. אם מדביקים קישור שיתוף של Google Maps, הוא נשאר טקסט רגיל בשדה הכתובת, לכן מומלץ להדביק ב-`Address` את הכתובת הכתובה ולשמור קישורים ישירים (אם צריך) בעמודת `Notes`.
* **אילו עמודות חשוב למלא:** `Email` היא חובה. מילוי `Name` (או `Given Name` ו-`Family Name`) עוזר לזהות אנשי קשר בייצוא. שאר הכותרות הן אופציונליות ומשמשות רק לשמירה על תאימות עם Google Contacts.
