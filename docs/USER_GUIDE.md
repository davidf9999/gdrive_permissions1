...
*   **How to Use:** Enter **exactly one valid email address per row** in Column A (`Email`). The header matches the column name Google Contacts expects (`Email`), so you can export or copy a user sheet directly into a Contacts import without renaming anything. If a cell contains multiple addresses or anything other than a single valid email, the script will log an error and ignore that entry. Optional Column B (`Disabled`) lets you temporarily exclude a user from receiving permissions—set it to `TRUE`, `YES`, or check the box to disable the user without deleting their row. Additional Google Contacts-compatible headers are provided for convenient exports/imports: `Name`, `Given Name`, `Family Name`, `Phone`, `Notes`, `Address`, `Organization`, `Title`, `Birthday`, and `Group Membership`.
    *   **Adding addresses or map links:** Google Contacts expects the `Address` column to contain a regular mailing address (street, city, etc.). When you import the CSV, Contacts automatically turns that text into a clickable map pin. If you paste a Google Maps share URL, Contacts keeps it as plain text in the address field, so use the written address in `Address` and keep any direct share links in `Notes` if you still need them.
*   **Which columns to fill:** `Email` is essential. Filling `Name` (or `Given Name` and `Family Name`) makes contact exports friendlier. The other headers are optional and available for consistency with Google Contacts but are not required by the script.
