...
function syncAdmins(options = {}) {
  ...
  try {
    ...
    if (emailsToAdd.length === 0 && (addOnly || emailsToRemove.length === 0)) {
      ...
      log_('Admin list is already up to date (add-only mode).');
      if (!silentMode) ui.alert('Admin list is already up to date. No changes were needed.\nAdmins group synced to ' + adminGroupEmail + '.');
      return;
    }
    ...
    if (!silentMode) ui.alert('Admin sync complete.\nAdmins group synced to ' + adminGroupEmail + '.');
